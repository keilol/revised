...
local SmoothModes = {
    ...
    Elastic = function(t)
        return math.sin(-13*(t+1)*math.pi/2) * 2^( -10 * t ) + 1
    end,

    Spring = function(t)
        return 1 - (math.cos(t * math.pi * 4) * math.exp(-4 * t))
    end
}
...
UIS.InputBegan:Connect(function(input, gpe)
    ...
    if input.KeyCode == Enum.KeyCode.E then
        aimbot_holding = not aimbot_holding  -- toggle state

        if aimbot_holding then
            -- Aimbot turning ON
            if sticky_enabled then
                -- If sticky and valid, keep existing target
                if sticky_target
                    and sticky_target.Character
                    and sticky_target.Character:FindFirstChild("Humanoid")
                    and sticky_target.Character.Humanoid.Health > 0
                then
                    currentTarget = sticky_target
                    return
                end

                -- Otherwise get new sticky target
...
-- Remove old InputEnded code completely (mouse button code).

-- ======================
-- Silent Aim Hook (Correct Da Hood Version)
-- ======================
local MainEvent = ReplicatedStorage:FindFirstChild("MainEvent")

if MainEvent and getrawmetatable then
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        -- Only modify UpdateMousePosI2
        if self == MainEvent 
            and method == "FireServer"
            and args[1] == "UpdateMousePosI2"
            and silent_enabled
            and silentTarget
            and silentTarget.Character
        then
            ...
            part = part or char:FindFirstChild("Head")
            if not part then
                return old(self, ...)
            end

            -- accuracy check
            local chance = math.clamp(tonumber(silent_hit_chance) or 100, 0, 100)
            ...
            if silent_prediction_enabled and HRP then
                pos = pos + HRP.Velocity * silent_prediction_amount
            end
    ...
    warn("[SilentAim] Hook could not be created.")
end
...
-- WALL CHECK: true if target part visible
local function IsVisible(origin, targetPart)
    ...
    params.FilterDescendantsInstances = {targetPart.Parent} -- ignore target char
...
RunService.RenderStepped:Connect(function(dt)
    ...
    -- Auto-unlock if sticky target invalid
if sticky_enabled and sticky_target then
    ...
    if (not sticky_target.Character)
        or (not hum)
        or hum.Health <= 0
    then
        sticky_target = nil
...
-- WALL CHECK: auto-unlock if not visible
-- WALL CHECK: unlock if invisible
if wallcheck_enabled then

    -- If we have a target, check visibility
    if currentTarget then
        local checked = UpdateTracking(Camera, currentTarget)

        -- Invisible → unlock but keep sticky ref
        if not checked then
            if sticky_enabled then
                sticky_target = currentTarget  -- remember this target
            end
            currentTarget = nil
        end

    -- We have no target BUT sticky mode remembers one
    elseif sticky_enabled and sticky_target then
        -- Check if sticky_target is visible again
...
if sticky_enabled and sticky_target then
    local hum = sticky_target.Character 
        and sticky_target.Character:FindFirstChild("Humanoid")
...
currentTarget = UpdateTracking(Camera, currentTarget)
    ...
    if aimbotOn and aimbot_holding and currentTarget and currentTarget.Character then
        ...
        if target_part_name == "Nearest part" then
            targetPart = GetNearestPart(char)
        else
            targetPart = char:FindFirstChild(target_part_name)
        end
